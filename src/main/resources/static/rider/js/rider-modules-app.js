(function () {
    ...
    async function initCurrent() {
        ...
        box.innerHTML = '<div class="mt-empty">正在加载当前配送...</div>';

        try {
            state.deliveringOrders = await RiderApp.request('/rider/orders/my?status=3');
            renderCurrent(state.deliveringOrders || []);
        } catch (e) {
            box.innerHTML = `<div class="mt-empty">${RiderApp.escapeHtml(e.message || '加载失败')}</div>`;
        }
    }

    async function initTips() {
        const box = RiderApp.$('#moduleContent');
        box.innerHTML = '<div class="mt-empty">正在加载打赏订单...</div>';
    ...
    function renderRecommend(orders) {
        ...
        if (!orders.length) {
            box.innerHTML = '<div class="mt-empty">暂无可接订单</div>';
            return;
        }
        ...
        box.innerHTML = orders.map((order, index) => {
            ...
            return `
        <article class="mt-order-card ${index === 0 ? 'nearest' : ''}">
          <div class="mt-order-head">
            <b>${index === 0 ? '最近 · ' : ''}#${RiderApp.escapeHtml(id)} · ${RiderApp.escapeHtml(merchant)}</b>
            <span class="mt-tag ${urged ? 'hot' : ''}">${urged ? '用户已催单' : '普通订单'}</span>
          </div>

          <div class="mt-route-mini">
          ...
          <div class="mt-order-head">
            <span class="mt-tag">距离优先推荐</span>
            <b>${RiderApp.formatMoney(total)}</b>
          </div>

          <div class="mt-actions">
            <button class="green" data-action="accept" data-id="${RiderApp.escapeHtml(id)}">立即接单</button>
            <a class="mt-link-btn" href="/rider/orders.html">进入订单大厅</a>
          </div>
        </article>
      `;
        }).join('');

        box.querySelectorAll('button[data-action="accept"]').forEach(btn => {
    ...
    function renderCurrent(orders) {
        ...
        if (!orders.length) {
            box.innerHTML = '<div class="mt-empty">当前没有配送中的订单</div>';
            return;
        }
    ...
    function renderTips() {
        ...
        if (!tipped.length) {
            box.innerHTML = '<div class="mt-empty">暂无用户打赏订单</div>';
            return;
        }
        ...
        <article class="mt-order-card">
          <div class="mt-order-head">
            <b>🎁 #${RiderApp.escapeHtml(id)} · ${RiderApp.escapeHtml(merchant)}</b>
            <span class="mt-tag">${status === 4 ? '已完成' : '配送中'}</span>
          </div>

          <p>${RiderApp.escapeHtml(address || '暂无地址')}</p>

          <div class="mt-order-head">
            <span class="mt-tag">用户打赏</span>
    ...
    function renderWaitCooking(orders) {
        ...
        <article class="mt-order-card">
          <div class="mt-order-head">
            <b>#${RiderApp.escapeHtml(id)} · ${RiderApp.escapeHtml(merchant)}</b>
            <span class="mt-tag hot">制作中</span>
          </div>
